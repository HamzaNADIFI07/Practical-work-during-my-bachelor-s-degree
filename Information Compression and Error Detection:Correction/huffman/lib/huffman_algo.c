...
int create_huffman_forest(int occurrences[], huffman_tree_p  forest[]) {
  ...
  for(i = 0; i < ALPHABET_SIZE; i++){
    if(occurrences[i] > 0){
      forest[x] = create_huffman_tree(i,occurrences[i]);
      x++;
    }
  }
  return x;
...
void huffman_coding(char *in_filename, char *out_filename, int verbose) {
  ...
  /*1er lecture pour compter les occurrence*/
  ...
  /* Écriture des occurrences dans le fichier compressé */
  ...
  /* Deuxième lecture du fichier et codage */
...
void huffman_decoding(char *in_filename, char *out_filename, int verbose){
  ...
  if(input == NULL){
    perror("Une erreur est survenue lors de l'ouverture du fichier en entrée");
    exit(1);
  }

  if(output == NULL){
    perror("Une erreur est survenue lors de l'ouverture du fichier en sortie");
    exit(1);
  }
  ...
  huffman_tree_p *hufTree = build_huffman_tree(forest,nb_leaves);


  if(verbose){
    print_huffman_tree(hufTree);
  }

  fseek(input,0,SEEK_SET);
  huffman_decode_file(input,output,hufTree,nb_leaves);
